<%@ WebHandler Language="C#" Class="GSiteMapVideo" %>
...
public class GSiteMapVideo : IHttpHandler
{
    ...
    public void GenerateXML(System.IO.Stream stream, int pageindex, int pagesize)
    {
        ...
        using (var writer = XmlWriter.Create(stream, settings))
        {
            ...
            var vnnVideoBll = new v_VideoBLL(con);
            //Lấy url tin phân trang vào sitemap
            var dt = vnnVideoBll.GetAllVideoForSiteMap("UpdatedDate,VideoTypeName,Title,VideoID", (pageindex - 1) * pagesize, pagesize, -1, 1, "", "");

            if (dt != null && dt.Count > 0)
            {
                foreach (var t in dt)
                {
                    WriteTag("0.5", "", t.UpdatedDate.ToString("yyyy-MM-dd"), UrlRoot + "/video/" + XuLyChuoi.ConvertToUnSign(t.VideoTypeName) + "/" + XuLyChuoi.ConvertToUnSign(t.Title) + "-hltw" + t.VideoID + ".aspx", writer);
                }
            }
