...
<asp:Repeater ID="rpVideoType" runat="server" OnItemDataBound="rpVideoType_ItemDataBound">
    <ItemTemplate>
        <div class="box-title">
            <div class="in2">
                <ul class="box-tab clear">
                    <li>
                        <h2>
                            <a class="cat-link" title='<%# Eval("Description") %>' href='<%# CurrentPage.UrlRoot + "/video/" + XuLyChuoi.ConvertToUnSign(Eval("VideoTypeName").ToString()) + "/hltw" + Eval("VideoTypeID") + ".aspx" %>'>
                                <%# Eval("videotypename") %></a></h2>
                    </li>
                </ul>
            </div>
        </div>
