...
public class ImageMedia
{
    ...
    public ImageMedia Resize(int? width, int? height)
    {
        ...
        var scale = Math.Min(width.Value / (double)_source.PixelWidth, height.Value / (double)_source.PixelHeight);
        if (scale >= 1)
            return this;
    ...
    public ImageMedia Crop(int? width, int? height)
    {
        ...
        var scale = Math.Max(width.Value / (double)_source.PixelWidth, height.Value / (double)_source.PixelHeight);
        if (scale < 1)
            _source = new TransformedBitmap(_source, new ScaleTransform(scale, scale, 0, 0));
    ...
    public byte[] ToByteArray(string format)
    {
        ...
        {
            byte[] data = null;
            var worker = new System.Threading.Thread(delegate(object obj)
            {
                var encoder = GetEncoder(format);
                try
                {
                    var meta = new BitmapMetadata(format) { Copyright = Copyright };
    ...
    private BitmapEncoder GetEncoder(string format)
    {
        if (format == "jpg")
        {
            var res = new JpegBitmapEncoder { QualityLevel = 85 };
            return res;
        }
        if (format == "png")
    ...
    {
        var result = new ImageMedia
            {
                _source =
                    BitmapDecoder.Create(new MemoryStream(data), BitmapCreateOptions.PreservePixelFormat,
                                         BitmapCacheOption.None).Frames[0],
                _data = data
            };
        try { result.Copyright = ((BitmapMetadata)result._source.Metadata).Copyright; }
        catch
        { }
        return result;
    }
