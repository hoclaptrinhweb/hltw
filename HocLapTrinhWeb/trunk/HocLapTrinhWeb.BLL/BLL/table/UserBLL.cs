...
{
    public class UserBLL : ClassBase
    {
        #region Variable
        ...
        public UserBLL(Connection conn)
        {
            IConnect = conn;
        }

        public dsHocLapTrinhWeb.tbl_UserRow GetUserByID(int id)
        {
            var isOpen = false;
            try
            {
        ...
        /// <returns></returns>
        public dsHocLapTrinhWeb.tbl_UserRow GetUserByEmail(string email)
        {
            var isOpen = false;
            try
            {
                ...
                {
                    var dt = new dsHocLapTrinhWeb.tbl_UserDataTable();
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt) { WhereClause = dt.EmailColumn.ColumnName + "=@1" };
                    _classBaseDAL.ClearParams();
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Add(dsHocLapTrinhWeb.tbl_UserDataTable dt)
        {
            bool isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    dt.AcceptChanges();
                    foreach (dsHocLapTrinhWeb.tbl_UserRow row in dt.Rows)
                        row.SetAdded();
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Update(dsHocLapTrinhWeb.tbl_UserDataTable dt)
        {
            var isOpen = false;
            try
            {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Update(dsHocLapTrinhWeb.tbl_UserDataTable dt, params string[] columnsName)
        {
            var isOpen = false;
            try
            {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Delete(dsHocLapTrinhWeb.tbl_UserDataTable dt)
        {
            bool isOpen = false;
            try
            {
        ...
        public bool Delete(ArrayList arrayID)
        {
            ...
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    _classBaseDAL.ClearParams();
                    var strWhereClause = dt.UserIDColumn.ColumnName + " in (";
