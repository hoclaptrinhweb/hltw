...
{
    public class UpCategoryForumBLL : ClassBase
    {
        #region Variable
        ...
        public UpCategoryForumBLL(Connection conn)
        {
            IConnect = conn;
        }

        public dsHocLapTrinhWeb.up_tbl_CategoryForumRow GetCategoryForumByID(int id)
        {
            var isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt)
                        {WhereClause = dt.CategoryForumIDColumn.ColumnName + "=@id"};
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Add(dsHocLapTrinhWeb.up_tbl_CategoryForumDataTable dt)
        {
            bool isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    dt.AcceptChanges();
                    foreach (dsHocLapTrinhWeb.up_tbl_CategoryForumRow row in dt.Rows)
                        row.SetAdded();
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Update(dsHocLapTrinhWeb.up_tbl_CategoryForumDataTable dt)
        {
            bool isOpen = false;
            try
            {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Delete(dsHocLapTrinhWeb.up_tbl_CategoryForumDataTable dt)
        {
            bool isOpen = false;
            try
            {
        ...
        public bool Delete(int id)
        {
            ...
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    _classBaseDAL.ClearParams();
                    _classBaseDAL.WhereClause = dt.CategoryForumIDColumn.ColumnName + "=@1";
        ...
        public bool Delete(ArrayList arrayID)
        {
            ...
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    _classBaseDAL.ClearParams();
                    var strWhereClause = dt.CategoryForumIDColumn.ColumnName + " in (";
