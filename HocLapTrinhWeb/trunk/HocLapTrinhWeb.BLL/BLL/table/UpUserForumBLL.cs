...
{
    public class UpUserForumBLL : ClassBase
    {
        #region Variable
        ...
        public UpUserForumBLL(Connection conn)
        {
            IConnect = conn;
        }

        public dsHocLapTrinhWeb.up_tbl_UserForumRow GetUserForumByID(int id)
        {
            var isOpen = false;
            try
            {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Add(dsHocLapTrinhWeb.up_tbl_UserForumDataTable dt)
        {
            bool isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    dt.AcceptChanges();
                    foreach (dsHocLapTrinhWeb.up_tbl_UserForumRow row in dt.Rows)
                        row.SetAdded();
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Update(dsHocLapTrinhWeb.up_tbl_UserForumDataTable dt)
        {
            bool isOpen = false;
            try
            {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Delete(dsHocLapTrinhWeb.up_tbl_UserForumDataTable dt)
        {
            var isOpen = false;
            try
            {
        ...
        public bool Delete(int id)
        {
            ...
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    _classBaseDAL.ClearParams();
                    _classBaseDAL.WhereClause = dt.UserForumIDColumn.ColumnName + "=@1";
        ...
        public bool Delete(ArrayList arrayID)
        {
            ...
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    _classBaseDAL.ClearParams();
                    var strWhereClause = dt.UserForumIDColumn.ColumnName + " in (";
