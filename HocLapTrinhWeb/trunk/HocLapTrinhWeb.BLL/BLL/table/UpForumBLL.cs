...
{
    public class UpForumBLL : ClassBase
    {
        #region Variable
        ...
        public UpForumBLL(Connection conn)
        {
            IConnect = conn;
        }

        public dsHocLapTrinhWeb.up_tbl_ForumRow GetForumByID(int id)
        {
            var isOpen = false;
            try
            {
        ...
        public dsHocLapTrinhWeb.up_tbl_ForumRow GetForumByName(string name)
        {
            var isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt)
                        {WhereClause = dt.ForumNameColumn.ColumnName + "=@Name"};
                    _classBaseDAL.ClearParams();
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Add(dsHocLapTrinhWeb.up_tbl_ForumDataTable dt)
        {
            bool isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    dt.AcceptChanges();
                    foreach (dsHocLapTrinhWeb.up_tbl_ForumRow row in dt.Rows)
                        row.SetAdded();
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Update(dsHocLapTrinhWeb.up_tbl_ForumDataTable dt)
        {
            bool isOpen = false;
            try
            {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Delete(dsHocLapTrinhWeb.up_tbl_ForumDataTable dt)
        {
            bool isOpen = false;
            try
            {
        ...
        public bool Delete(int id)
        {
            ...
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    _classBaseDAL.ClearParams();
                    _classBaseDAL.WhereClause = dt.ForumIDColumn.ColumnName + "=@1";
        ...
        public bool Delete(ArrayList arrayID)
        {
            ...
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    _classBaseDAL.ClearParams();
                    var strWhereClause = dt.ForumIDColumn.ColumnName + " in (";
