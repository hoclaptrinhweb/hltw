...
{
    public class ContactBLL : ClassBase
    {
        #region Variable
        ...
        public ContactBLL(Connection conn)
        {
            IConnect = conn;
        }

        public dsHocLapTrinhWeb.tbl_ContactRow GetContactByID(int id)
        {
            var isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt)
                        {WhereClause = dt.ContactIDColumn.ColumnName + "=@id"};
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Add(dsHocLapTrinhWeb.tbl_ContactDataTable dt)
        {
            var isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    dt.AcceptChanges();
                    foreach (dsHocLapTrinhWeb.tbl_ContactRow row in dt.Rows)
                        row.SetAdded();
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Update(dsHocLapTrinhWeb.tbl_ContactDataTable dt)
        {
            var isOpen = false;
            try
            {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Delete(dsHocLapTrinhWeb.tbl_ContactDataTable dt)
        {
            var isOpen = false;
            try
            {
        ...
        public bool Delete(int id)
        {
            ...
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    _classBaseDAL.ClearParams();
                    _classBaseDAL.WhereClause = dt.ContactIDColumn.ColumnName + "=@1";
        ...
        public bool Delete(ArrayList arrayID)
        {
            ...
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    _classBaseDAL.ClearParams();
                    string strWhereClause = dt.ContactIDColumn.ColumnName + " in (";
