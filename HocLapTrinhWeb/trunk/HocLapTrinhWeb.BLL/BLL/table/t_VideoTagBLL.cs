...
{
    public class t_VideoTagBLL : ClassBase
    {
        #region Variable
        ...
        public t_VideoTagBLL(Connection conn)
        {
            IConnect = conn;
        }

        public dsHocLapTrinhWeb.tbl_VideoTagRow Get_VideoTagByID(int id)
        {
            var isOpen = false;
            try
            {
                ...
                {
                    var dt = new dsHocLapTrinhWeb.tbl_VideoTagDataTable();
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt) { WhereClause = dt.VideoTagIDColumn.ColumnName + "=@id" };
        ...
        public dsHocLapTrinhWeb.tbl_VideoTagDataTable Get_VideoTagByID(int newsid, ArrayList notTagid)
        {
            var isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    {
                        strWhereClause += "@" + i + ",";
                        _classBaseDAL.AddParams(i.ToString(), SqlDbType.Int, int.Parse(notTagid[i - 1].ToString()), ParameterDirection.Input);
                    }
        ...
        public dsHocLapTrinhWeb.tbl_VideoTagRow Get_VideoTagByID(int tagid, int newsid)
        {
            var isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL.WhereClause += " and " + dt.VideoIDColumn.ColumnName + "=@newsid";
                    _classBaseDAL.AddParams("@newsid", SqlDbType.Int, newsid, ParameterDirection.Input);
                    if (_classBaseDAL.FillData(dt))
                    {
                        if (dt.Count == 0)
        ...
        public dsHocLapTrinhWeb.tbl_VideoTagRow GetVideoTagByDate(string strdate)
        {
            var isOpen = false;
            try
            {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Add(dsHocLapTrinhWeb.tbl_VideoTagDataTable dt)
        {
            var isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    dt.AcceptChanges();
                    foreach (dsHocLapTrinhWeb.tbl_VideoTagRow row in dt.Rows)
                        row.SetAdded();
        ...
        public bool Update(dsHocLapTrinhWeb.tbl_VideoTagDataTable dt)
        {
            var isOpen = false;
            try
            {
        ...
        public bool Update(dsHocLapTrinhWeb.tbl_VideoTagDataTable dt, params string[] columnsName)
        {
            var isOpen = false;
            try
            {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public bool Delete(dsHocLapTrinhWeb.tbl_VideoTagDataTable dt)
        {
            var isOpen = false;
            try
            {
        ...
        public bool Delete(int id)
        {
            ...
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    _classBaseDAL.ClearParams();
                    _classBaseDAL.WhereClause = dt.VideoTagIDColumn.ColumnName + "=@1";
        ...
        public bool Delete(ArrayList arrayID)
        {
            ...
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    ...
                    _classBaseDAL = new ClassBaseDAL(IConnect, dt);
                    _classBaseDAL.ClearParams();
                    string strWhereClause = dt.VideoTagIDColumn.ColumnName + " in (";
