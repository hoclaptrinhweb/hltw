...
{
    public class vnn_UpForumBLL : UpForumBLL
    {
        #region Variable
        ...
        public vnn_UpForumBLL(Connection conn)
            : base(conn)
        {
        ...
        /// <summary>
        /// Load Forum len gridview
        /// </summary>
        /// <param name="startRowIndex"></param>
        /// <param name="maximumRows"></param>
        /// <returns></returns>
        public vnn_dsHocLapTrinhWeb.vnn_vw_UpForumDataTable GetAllForumForGridView(int startRowIndex, int maximumRows)
        {
            var isOpen = false;
            try
            {
                ...
                {
                    var dt = new vnn_dsHocLapTrinhWeb.vnn_vw_UpForumDataTable();
                    _ClassBaseDAL = new ClassBaseDAL(IConnect, dt)
                        {
                            StartRow = startRowIndex,
                            MaxRows = maximumRows,
                            OrderByClause = dt.AlexaColumn.ColumnName + " asc"
                        };
        ...
        public vnn_dsHocLapTrinhWeb.vnn_vw_UpForumDataTable GetAllForumForGridView()
        {
            var isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    var dt = new vnn_dsHocLapTrinhWeb.vnn_vw_UpForumDataTable();
                    _ClassBaseDAL = new ClassBaseDAL(IConnect, dt)
                        {OrderByClause = dt.ForumIDColumn.ColumnName + " desc"};
                    if (_ClassBaseDAL.FillData(dt))
                        return dt;
                    AddMessage("ERR-000006", "Tải dữ liệu không thành công." + _ClassBaseDAL.getMessage(), _ClassBaseDAL.getMsgNumber());
                    return null;
        ...
        /// </summary>
        /// <returns></returns>
        public int GetAllForumRowCount()
        {
            bool isOpen = false;
            try
            {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public vnn_dsHocLapTrinhWeb.vnn_vw_UpForumRow GetForumByID(int id)
        {
            bool isOpen = false;
            try
            {
                ...
                {
                    var dt = new vnn_dsHocLapTrinhWeb.vnn_vw_UpForumDataTable();
                    _ClassBaseDAL = new ClassBaseDAL(IConnect, dt) {WhereClause = dt.ForumIDColumn.ColumnName + "=@id"};
        ...
        public vnn_dsHocLapTrinhWeb.vnn_vw_UpForumDataTable GetForumSearchName(string name, int top)
        {
            bool isOpen = false;
            try
            {
                if (OpenConnection(ref isOpen))
                {
                    var dt = new vnn_dsHocLapTrinhWeb.vnn_vw_UpForumDataTable();
                    _ClassBaseDAL = new ClassBaseDAL(IConnect, dt)
                        {Top = top, WhereClause = dt.ForumNameColumn.ColumnName + " like N'%" + name + "%'"};
                    if (_ClassBaseDAL.FillData(dt))
                    {
                        return dt;
                    }
        ...
        public vnn_dsHocLapTrinhWeb.vnn_vw_UpForumRow GetForumByName(string forumName)
        {
            var isOpen = false;
            try
            {
                ...
                {
                    var dt = new vnn_dsHocLapTrinhWeb.vnn_vw_UpForumDataTable();
                    _ClassBaseDAL = new ClassBaseDAL(IConnect, dt) {WhereClause = dt.ForumNameColumn.ColumnName + "=@1"};
                    _ClassBaseDAL.ClearParams();
                    _ClassBaseDAL.AddParams("@1", SqlDbType.VarChar, forumName, ParameterDirection.Input);
                    if (_ClassBaseDAL.FillData(dt))
                    {
                        if (dt.Count == 0)
                        {
